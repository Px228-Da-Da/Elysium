...
impl App {
    ...
    pub(in crate::app) fn ui_now_playing(&mut self, ui: &mut egui::Ui) {
        ...
        let lab_rect = Rect::from_center_size(
            pos2(area.right() - 105.0, area.top() + 40.0),
            vec2(64.0, 28.0),
        );
        let lab_resp = ui.interact(lab_rect, ui.id().with("now_playing_lab"), egui::Sense::click());
        let lab_bg = if lab_resp.hovered() {
            Color32::from_rgb(60, 24, 30)
        } else {
        ...
        ui.painter().rect(
            ...
            Rounding::same(14.0),
            lab_bg,
            Stroke::new(1.0, Color32::from_rgb(150, 40, 55)),
        );
        ui.painter().text(
            lab_rect.center(),
